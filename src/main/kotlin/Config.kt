...
object Config : AutoSavePluginConfig("Setting") {
    ...
    @ValueDescription(
        """
        使用群聊许可名单 当此项为空时则代表关闭了本插件服务 使用特殊的表达式 
        表达式名单:
        
        - `+` 增加对群聊的使用 后面为群聊号, 如`+114514`
        - `*` 对所有群聊都打开功能, 如`*`
        - `-` 禁止对群聊的使用 后面为群聊号, 如`-114514`
        
        
        表达式可以叠加， 如: 
        [*,-1919810] 则表达为支持所有群聊，但是不对群聊号为1919810的目标提供服务.
        [+1919810, -114514] 此表达式中`-114514` 是无效的，因为不含`*`项则代表只会对`+`的群聊处理
        [+1919810, -1919810] 此表达式中等同于无效，功能并未打开，因为`+`和`-`会相互抵消
    """
    )
    val allowlist: MutableList<String> by value(mutableListOf("*"))
